document.addEventListener("DOMContentLoaded", () => {
  ...
  function logSubmit(event) {
    ...
    // toDoList.innerHTML += `<li style=color:${color[select.value]}> ${input.value} <button id=${input.value}> X </button></li>`
   ...
   let editButton = document.getElementsByClassName('edit');
    ...
    for (let i=0; i < deleteButton.length; i++){
      // deleteButton[i].addEventListener('click', () => toDoList.removeChild(list[i]));
      // deleteButton[i].addEventListener('click', () => toDoList.removeChild(list[i]));
      console.log('hello');
    } 
